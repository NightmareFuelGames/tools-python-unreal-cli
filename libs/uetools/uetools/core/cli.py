...
from uetools.core.perf import show_timings, timeit
from uetools.core import print, debug
from uetools.core.util import deduce_project_plugin
...
def build_parser(commands):
    with timeit("build_parser"):
        ...
        from uetools.globals import SHOULD_PROFILE
        if SHOULD_PROFILE:
            parser.add_argument("-zyx", default=True, action=DumpParserAction, help="")
        else:
        ...
        parser.add_argument(
            "-xyz",
            action="store_true", default=False, help="Show perf timing",
        )
...
def build_parser_typed():
    zyx: bool = False # DumpParserAction **
    engine_version: str = "5.4.0"
    xyz: bool = False #store_true ** Show perf timing
    command: str = "uat"
...
should_profile = "-xyz" in sys.argv or SHOULD_PROFILE
