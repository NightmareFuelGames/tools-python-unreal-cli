...
def load_conf() -> dict:
    ...
    # pylint: disable=globals-statement
    global LATEST_CONF
...
def get_selected_engine_version() -> str:
...
def retrieve_exact_engine_version(engine_folder, default=None, ignore_patch=True) -> str:
...
def select_engine_version(version)  -> None:
...
def engine_folder(version=None) -> str:
...
def is_ci() -> bool:  # todo: Check here for perforce
    return os.getenv("GITHUB_ACTIONS") == "true" or os.getenv("CI") == "true"


def get_version_tag(path, default="1.0") -> str:
